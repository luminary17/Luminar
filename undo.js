...
const Vocab = (() => {
  ...
  function openFolderPage(fid, fname, type, push = true) {
    ...
    pg.innerHTML = `
      <button class="back-btn" onclick="Vocab.closeFolderPage('${type}','${fid}')">← Назад</button>
    ...
    if (push) history.pushState({}, '', `/edu/${type}/vocab/${encodeURIComponent(fid)}/`);
  }

  function closeFolderPage(type, fid, push = true) {
    document.getElementById(`${type}-folder-cards`).style.display = 'block';
    document.querySelectorAll(`#${type}-folder-pages > div`).forEach(d => d.style.display = 'none');
    if (push) history.pushState({}, '', `/edu/${type}/vocab/`);
  }
